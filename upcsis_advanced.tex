...
%%%%%%%%%%%%%为了让长到换行的标题能够在第一个字的位置对齐：
%%%%%%%%%%%%%请在断行的位置加上   \\\hspace{4.1em}  如：

% \title{这是一个很，特别，尤其，过分，极为，十分，\\ \hspace{4.1em}格外超级，无敌长的标题}
...
\newtheorem{mdef}{\quad\quad 定义}[section]　%按一级标题编号
\theorembodyfont{\songti}
\newtheorem{mthe}{\quad\quad 定理}[section]
\theorembodyfont{\songti}
\newtheorem*{mpro}{\quad\quad 证明} %不编号
\theorembodyfont{\songti}
